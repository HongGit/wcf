...
using static System.ServiceModel.Description.MetadataImporter;
...
namespace System.ServiceModel.Description
{
    ...
    {
        readonly Dictionary<WsdlNS.NamedItem, WsdlImportException> _importErrors = new Dictionary<WsdlNS.NamedItem, WsdlImportException>();
        bool _isFaulted = false;

        readonly Dictionary<XmlQualifiedName, WsdlContractConversionContext> _importedPortTypes = new Dictionary<XmlQualifiedName, WsdlContractConversionContext>();
        readonly Dictionary<XmlQualifiedName, WsdlEndpointConversionContext> _importedBindings = new Dictionary<XmlQualifiedName, WsdlEndpointConversionContext>();
        readonly Dictionary<WsdlNS.Port, ServiceEndpoint> _importedPorts = new Dictionary<WsdlNS.Port, ServiceEndpoint>();

        readonly KeyedByTypeCollection<IWsdlImportExtension> _wsdlExtensions;

        readonly WsdlNS.ServiceDescriptionCollection _wsdlDocuments = new WsdlNS.ServiceDescriptionCollection();
        //readonly XmlSchemaSet _xmlSchemas = WsdlExporter.GetEmptySchemaSet();
        readonly Dictionary<string, XmlElement> _policyDocuments = new Dictionary<string, XmlElement>();
        internal readonly Dictionary<string, string> _warnings = new Dictionary<string, string>();
        private readonly XmlSchemaSet _xmlSchemas = null;
        WsdlPolicyReader _wsdlPolicyReader;

        bool _beforeImportCalled = false;
            ...
            : base(policyImportExtensions, quotas)
        {
            ...
            ProcessMetadataDocuments(metadata.MetadataSections);
        }
        ...
        {
            return PolicyReader.ResolvePolicyReference(policyReference, contextAssertion);
        }

        public override Collection<ContractDescription> ImportAllContracts()
        {
            if (_isFaulted)
                //throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SecurityRuleSet.Format(SR.WsdlImporterIsFaulted)));
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.WsdlImporterIsFaulted));
        ...
        {
            if (_isFaulted)
                //throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR.WsdlImporterIsFaulted));
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.WsdlImporterIsFaulted));

            EnsureBeforeImportCalled();
            ServiceEndpointCollection endpoints = new ServiceEndpointCollection();
            foreach (WsdlNS.Port wsdlPort in SoapInPolicyWorkaroundHelper.GetAllPorts(WsdlDocuments))
            {
        ...
        public Collection<System.ServiceModel.Channels.Binding> ImportAllBindings()
        {
            ...
            EnsureBeforeImportCalled();
            Collection<System.ServiceModel.Channels.Binding> bindings = new Collection<System.ServiceModel.Channels.Binding>();
            foreach (WsdlNS.Binding wsdlBinding in SoapInPolicyWorkaroundHelper.GetAllBindings(WsdlDocuments))
            {
        ...
        public System.ServiceModel.Channels.Binding ImportBinding(WsdlNS.Binding wsdlBinding)
        {
            if (_isFaulted)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.WsdlImporterIsFaulted));
        ...
        public ServiceEndpointCollection ImportEndpoints(WsdlNS.PortType wsdlPortType)
        {
            ...
            if (IsBlackListed(wsdlPortType))
                throw SoapInPolicyWorkaroundHelper.CreateAlreadyFaultedException(_importErrors, wsdlPortType);
            else
                ImportWsdlPortType(wsdlPortType, WsdlPortTypeImportOptions.ReuseExistingContracts, ErrorBehavior.RethrowExceptions);

            ServiceEndpointCollection endpoints = new ServiceEndpointCollection();

            foreach (WsdlNS.Binding wsdlBinding in SoapInPolicyWorkaroundHelper.FindBindingsForPortType(wsdlPortType, WsdlDocuments))
                if (!IsBlackListed(wsdlBinding))
        ...
        internal ServiceEndpointCollection ImportEndpoints(ContractDescription contract)
        {
            ...
            foreach (WsdlNS.Binding wsdlBinding in SoapInPolicyWorkaroundHelper.FindBindingsForContract(contract, WsdlDocuments))
                if (!IsBlackListed(wsdlBinding))
                    foreach (ServiceEndpoint endpoint in ImportEndpoints(wsdlBinding))
                        endpoints.Add(endpoint);
        ...
        public ServiceEndpointCollection ImportEndpoints(WsdlNS.Binding wsdlBinding)
        {
            ...
            if (IsBlackListed(wsdlBinding))
                throw SoapInPolicyWorkaroundHelper.CreateAlreadyFaultedException(_importErrors, wsdlBinding);
            else
                ImportWsdlBinding(wsdlBinding, ErrorBehavior.RethrowExceptions);

            ServiceEndpointCollection endpoints = new ServiceEndpointCollection();

            foreach (WsdlNS.Port wsdlPort in SoapInPolicyWorkaroundHelper.FindPortsForBinding(wsdlBinding, WsdlDocuments))
                if (!IsBlackListed(wsdlPort))
                {
                    ServiceEndpoint endpoint = ImportWsdlPort(wsdlPort, ErrorBehavior.DoNotThrowExceptions);
                    if (endpoint != null)
        ...
        {
            if (IsBlackListed(wsdlPortType))
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(SoapInPolicyWorkaroundHelper.CreateAlreadyFaultedException(_importErrors, wsdlPortType));
            ...
            if (importOptions == WsdlPortTypeImportOptions.IgnoreExistingContracts || !TryFindExistingContract(wsdlPortTypeQName, out contractDescription))
            {
                ...
                catch (Exception e)
                {
                    ...
                    SoapInPolicyWorkaroundHelper.LogImportError(wsdlPortType, wie, _importErrors, Errors);
        ...
        WsdlEndpointConversionContext ImportWsdlBinding(WsdlNS.Binding wsdlBinding, ErrorBehavior errorBehavior)
        {
            ...
            if (IsBlackListed(wsdlBinding))
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(SoapInPolicyWorkaroundHelper.CreateAlreadyFaultedException(_importErrors, wsdlBinding));
            ...
            if (!_importedBindings.TryGetValue(wsdlBindingQName, out bindingEndpointContext))
            {
                ...
                catch (Exception e)
                {
                    ...
                    SoapInPolicyWorkaroundHelper.LogImportError(wsdlBinding, wie, _importErrors, Errors);
                    if (errorBehavior == ErrorBehavior.RethrowExceptions)
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(wie);
                    else
                        return null;
        ...
        {
            if (IsBlackListed(wsdlPort))
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(SoapInPolicyWorkaroundHelper.CreateAlreadyFaultedException(_importErrors, wsdlPort));
            ...
            if (!_importedPorts.TryGetValue(wsdlPort, out endpoint))
            {
                ...
                try
                {
                    WsdlNS.Binding wsdlBinding = this._wsdlDocuments.GetBinding(wsdlPort.Binding);
                ...
                catch (Exception e)
                {
                    ...
                    SoapInPolicyWorkaroundHelper.LogImportError(wsdlPort, wie, _importErrors, Errors);
                    if (errorBehavior == ErrorBehavior.RethrowExceptions)
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(wie);
                    else
                        return null;
        ...
        System.ServiceModel.Channels.Binding CreateBinding(WsdlEndpointConversionContext endpointContext, XmlQualifiedName bindingQName)
        {
            try
            {
                // either the wsdl:binding has already been imported or the wsdl:port has policy
                BindingElementCollection bindingElements = ImportPolicyFromWsdl(endpointContext, PolicyReader, base._quotas, _warnings, Errors);
                CustomBinding binding = new(bindingElements);
            ...
            foreach (MetadataSection doc in metadataSections)
            {
                try
                {
                    ...
                    if (doc.Dialect == MetadataSection.PolicyDialect)
                    {
                        ...
                        {
                            SoapInPolicyWorkaroundHelper.LogImportWarning(_warnings, SRP.Format(SRP.PolicyDocumentMustHaveIdentifier), Errors);
                        }
                        else
                        {
        ...
        T TryConvert<T>(MetadataSection doc)
        {
            ...
            catch (InvalidCastException)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new NotSupportedException(
                    SRP.Format(SRP.SFxBadMetadataDialect, doc.Identifier, doc.Dialect, typeof(T).FullName, doc.GetType().FullName)));
            }
        }
        ...
        void EnsureBeforeImportCalled()
        {
            ...
            {
                foreach (IWsdlImportExtension extension in this._wsdlExtensions)
                {
                    try
                    {
                        extension.BeforeImport(this._wsdlDocuments, this._xmlSchemas, this._policyDocuments.Values);
                    }
                    //#pragma warning suppress 56500 // covered by FxCOP
                    catch (Exception e)
                    {
                        ...
                        if (Fx.IsFatal(e))
                            throw;
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(SoapInPolicyWorkaroundHelper.CreateBeforeImportExtensionException(extension, e));
                    }
                }
        ...
        void CallImportContract(WsdlContractConversionContext contractConversionContext)
        {
            foreach (IWsdlImportExtension extension in this._wsdlExtensions)
                ...
                catch (Exception e)
                {
                    if (Fx.IsFatal(e))
                        throw;
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(SoapInPolicyWorkaroundHelper.CreateExtensionException(extension, e));
                }
        }
        ...
        void VerifyImportedExtensions(WsdlNS.NamedItem item)
        {
            foreach (object ext in item.Extensions)
            {
                ...
                {
                    string errorMsg = SRP.Format(SRP.RequiredWSDLExtensionIgnored, qName.Name, qName.Namespace);
                    WsdlImportException wie = WsdlImportException.Create(item, new InvalidOperationException(errorMsg));
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(wie);
                }
                else
                {
                    string xPath = SoapInPolicyWorkaroundHelper.CreateXPathString(item);
                    string errorMsg = SRP.Format(SRP.OptionalWSDLExtensionIgnored, qName.Name, qName.Namespace, xPath);
                    Errors.Add(new MetadataConversionError(errorMsg, true));
                }
            }
        }
        ...
        XmlQualifiedName GetUnhandledExtensionQName(object extension, WsdlNS.NamedItem item)
        {
            ...
            WsdlImportException wie = WsdlImportException.Create(item, new InvalidOperationException(SRP.Format(SRP.UnknownWSDLExtensionIgnored, extension.GetType().AssemblyQualifiedName)));
            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(wie);
        }

        internal BindingElementCollection ImportPolicyFromWsdl(WsdlEndpointConversionContext endpointContext, WsdlPolicyReader reader, MetadataImporterQuotas quotas, Dictionary<string, string> warnings, Collection<MetadataConversionError> errors)
        {
            PolicyAlternatives policyAlternatives = reader.GetPolicyAlternatives(endpointContext);
            IEnumerable<PolicyConversionContext> policyContexts = GetPolicyConversionContextEnumerator(endpointContext.Endpoint, policyAlternatives, quotas);
            ...
            foreach (PolicyConversionContext policyConversionContext in policyContexts)
            {
                ...
                {
                    SoapInPolicyWorkaroundHelper.AppendUnImportedPolicyErrorMessage(ref unImportedPolicyMessage, endpointContext, policyConversionContext);
                }

                if (++policyConversionContextsSeen >= quotas.MaxPolicyConversionContexts)
                {
                    break;
                }
            }
            ...
            if (firstContext != null)
            {
                ...
                firstContext.BindingElements.Insert(0, SoapInPolicyWorkaroundHelper.CollectUnrecognizedAssertions(firstContext, endpointContext));
                SoapInPolicyWorkaroundHelper.LogImportWarning(warnings, unImportedPolicyMessage.ToString(), errors);
                return firstContext.BindingElements;
            }
            // Consider: a /verbose option for svcutil...
            if (endpointContext.WsdlPort != null)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(WsdlImportException.Create(endpointContext.WsdlPort, new InvalidOperationException(SRP.Format(SRP.NoUsablePolicyAssertions))));
            }
            else
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(WsdlImportException.Create(endpointContext.WsdlBinding, new InvalidOperationException(SRP.Format(SRP.NoUsablePolicyAssertions))));
            }
        }
        ...
        internal static class Binding2DescriptionHelper
        {
            internal static OperationDescription FindOperationDescription(WsdlNS.OperationBinding wsdlOperationBinding, WsdlNS.ServiceDescriptionCollection wsdlDocuments, WsdlEndpointConversionContext endpointContext)
            {
                ...
                {
                    //TODO
                    //operation = SoapInPolicyWorkaroundHelper.FindOperationDescription(endpointContext.Endpoint.Contract, wsdlOperationBinding);
                    operation = null;
                }
                return operation;
            }
            ...
            static WsdlNS.Operation FindWsdlOperation(WsdlNS.OperationBinding wsdlOperationBinding, WsdlNS.ServiceDescriptionCollection wsdlDocuments)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(SRP.SFxInvalidWsdlBindingOpNoName, wsdlOperationBinding.Binding.Name)));
                }
                ...
                else
                {
                    ...
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(SRP.SFxInvalidWsdlBindingOpMismatch2, wsdlOperationBinding.Binding.Name, wsdlOperationBinding.Name)));
                }
            }
            ...
            //    {
            //        if (SoapInPolicyWorkaroundHelper.CompareOperations(operationDescription, contract, wsdlOperationBinding))
            //            return operationDescription;
            //    }

            //    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(SRP.UnableToLocateOperation2, wsdlOperationBinding.Name, contract.Name)));
            //}
        ...
        internal static class WSAddressingHelper
        {
            ...
            internal static EndpointAddress ImportAddress(WsdlNS.Port wsdlPort)
            {
                ...
                if (wsdlPort != null)
                {
                    ...
                    {
                        return EndpointAddress.ReadFrom(AddressingVersion.WSAddressing10, XmlDictionaryReader.CreateDictionaryReader(new XmlNodeReader(addressing10Element)));
                    }
                    if (addressing200408Element != null)
                    {
                        return EndpointAddress.ReadFrom(AddressingVersion.WSAddressingAugust2004, XmlDictionaryReader.CreateDictionaryReader(new XmlNodeReader(addressing200408Element)));
                    }
            ...
            internal static SupportedAddressingMode DetermineSupportedAddressingMode(MetadataImporter importer, PolicyConversionContext context)
            {
                ...
                if (addressingAssertion != null)
                {
                    ...
                    {
                        string message = SRP.Format(SRP.ElementRequired, MetadataStrings.Addressing10.MetadataPolicy.Prefix,
        ...
        internal static class SoapInPolicyWorkaroundHelper
        {
            ...
            static XmlDocument s_xmlDocument;
            ...
            internal static IEnumerable<WsdlNS.Binding> FindBindingsForPortType(WsdlNS.PortType wsdlPortType, WsdlNS.ServiceDescriptionCollection wsdlDocuments)
            {
                foreach (WsdlNS.Binding wsdlBinding in GetAllBindings(wsdlDocuments))
                {
            ...
            internal static IEnumerable<WsdlNS.Binding> FindBindingsForContract(ContractDescription contract, WsdlNS.ServiceDescriptionCollection wsdlDocuments)
            {
                XmlQualifiedName qName = WsdlNamingHelper.GetPortTypeQName(contract);
                foreach (WsdlNS.Binding wsdlBinding in GetAllBindings(wsdlDocuments))
                {
            ...
            internal static IEnumerable<WsdlNS.Port> FindPortsForBinding(WsdlNS.Binding binding, WsdlNS.ServiceDescriptionCollection wsdlDocuments)
            {
                foreach (WsdlNS.Port wsdlPort in SoapInPolicyWorkaroundHelper.GetAllPorts(wsdlDocuments))
                {
            ...
            internal static IEnumerable<WsdlNS.Binding> GetAllBindings(WsdlNS.ServiceDescriptionCollection wsdlDocuments)
            {
                foreach (WsdlNS.ServiceDescription wsdl in wsdlDocuments)
                {
                    foreach (WsdlNS.Binding wsdlBinding in wsdl.Bindings)
            ...
            internal static IEnumerable<WsdlNS.Port> GetAllPorts(WsdlNS.ServiceDescriptionCollection wsdlDocuments)
            {
                foreach (WsdlNS.ServiceDescription wsdl in wsdlDocuments)
                {
            ...
            internal static UnrecognizedAssertionsBindingElement CollectUnrecognizedAssertions(PolicyConversionContext policyContext, WsdlEndpointConversionContext endpointContext)
            {
            ...
            internal static void AppendUnImportedPolicyErrorMessage(ref StringBuilder unImportedPolicyMessage, WsdlEndpointConversionContext endpointContext, PolicyConversionContext policyContext)
            {
                if (unImportedPolicyMessage == null)
                {
                    unImportedPolicyMessage = new StringBuilder(SRP.Format(SRP.UnabletoImportPolicy));
                }
                else
                {
            ...
            {
                stringBuilder.AppendLine(SRP.Format(SRP.UnImportedAssertionList, CreateXPathString(item)));
            ...
            internal static string CreateXPathString(WsdlNS.NamedItem item)
            {
                if (item == null)
                    return SRP.Format(SRP.XPathUnavailable);
            ...
            internal static void GetXPathParameters(WsdlNS.NamedItem item, out string wsdlNs, out string localName, ref string nameValue, ref string rest)
            {
            ...
            internal static void LogImportWarning(Dictionary<string, string> warnings, string warningMessage, Collection<MetadataConversionError> errors)
            {
                ...
                errors.Add(new MetadataConversionError(warningMessage, true));
            }

            internal static void LogImportError(WsdlNS.NamedItem item, WsdlImportException wie, Dictionary<WsdlNS.NamedItem, WsdlImportException> importErrors, Collection<MetadataConversionError> errors)
            {
                string errormessage;
                if (wie.InnerException != null && (wie.InnerException is WsdlImportException))
                {
                    ...
                    string dependencyMessage = SRP.Format(SRP.WsdlImportErrorDependencyDetail, GetElementName(wieInner.SourceItem), GetElementName(item), CreateXPathString(wieInner.SourceItem));
                    errormessage = SRP.Format(SRP.WsdlImportErrorMessageDetail, GetElementName(item), CreateXPathString(wie.SourceItem), dependencyMessage);
                }
                else
                {
                    errormessage = SRP.Format(SRP.WsdlImportErrorMessageDetail, GetElementName(item), CreateXPathString(wie.SourceItem), wie.Message);
                }

                importErrors.Add(item, wie);
                errors.Add(new MetadataConversionError(errormessage, false));
            }

            internal static Exception CreateBeforeImportExtensionException(IWsdlImportExtension importer, Exception e)
            {
                string errorMessage = SRP.Format(SRP.WsdlExtensionBeforeImportError, importer.GetType().AssemblyQualifiedName, e.Message);
                return new InvalidOperationException(errorMessage, e);
            }

            internal static Exception CreateAlreadyFaultedException(Dictionary<WsdlNS.NamedItem, WsdlImportException> importErrors, WsdlNS.NamedItem item)
            {
                WsdlImportException innerException = importErrors[item];
                string warningMsg = SRP.Format(SRP.WsdlItemAlreadyFaulted, GetElementName(item));
                return new AlreadyFaultedException(warningMsg, innerException);
            }

            internal static Exception CreateExtensionException(IWsdlImportExtension importer, Exception e)
            {
                string errorMessage = SRP.Format(SRP.WsdlExtensionImportError, importer.GetType().FullName, e.Message);
        ...
        internal class WsdlImportException : Exception
        {
            WsdlNS.NamedItem _sourceItem;
            readonly string _xPath = null;

            WsdlImportException(WsdlNS.NamedItem item, Exception innerException)
                : base(string.Empty, innerException)
            {
                _xPath = SoapInPolicyWorkaroundHelper.CreateXPathString(item);
                _sourceItem = item;
            }
            ...
            {
                return XPath.StartsWith(SoapInPolicyWorkaroundHelper.CreateXPathString(item), StringComparison.Ordinal);
            }
            ...
            public override string Message
            {
                get
                {
                    Exception messageException = InnerException;
        ...
        {
            readonly WsdlImporter _importer;
            WsdlPolicyDictionary _policyDictionary;
            ...
            void LogPolicyNormalizationWarning(XmlElement contextAssertion, string warningMessage)
            {
                ...
                {
                    warningMsg.AppendLine(SRP.Format(SRP.XPathPointer, xPath));
                }
                else
                {
                    //
                    ...
                    //
                    warningMsg.AppendLine(SRP.Format(SRP.XPathPointer, SRP.Format(SRP.XPathUnavailable)));
                }
                WsdlImporter.SoapInPolicyWorkaroundHelper.LogImportWarning(_importer._warnings, warningMsg.ToString(), _importer.Errors);
            }
            ...
            IEnumerable<XmlElement> GetReferencedPolicy(WsdlNS.NamedItem item, WsdlNS.ServiceDescription wsdl)
            {
                ...
                foreach (string policyRef in GetPolicyReferenceUris(item, xPath))
                {
                    ...
                    {
                        StringBuilder warningMsg = new StringBuilder();
                        warningMsg.AppendLine(SRP.Format(SRP.UnableToFindPolicyWithId, policyRef));
                        warningMsg.AppendLine(SRP.Format(SRP.XPathPointer, xPath));
                        WsdlImporter.SoapInPolicyWorkaroundHelper.LogImportWarning(_importer._warnings, warningMsg.ToString(), _importer.Errors);
                        continue;
                    }
            ...
            IEnumerable<string> ReadPolicyReferenceElements(WsdlNS.NamedItem item, string xPath)
            {
                ...
                foreach (XmlElement element in policyReferences)
                {
                    ...
                    if (idRef == null)
                    {
                        ...
                        WsdlImporter.SoapInPolicyWorkaroundHelper.LogImportWarning(_importer._warnings, warningMsg, _importer.Errors);
                    }
            ...
            internal PolicyAlternatives GetPolicyAlternatives(WsdlEndpointConversionContext endpointContext)
            {
                ...
                if (endpointContext.WsdlPort != null)
                {
                    ...
                    policyAlternatives.EndpointAlternatives = PolicyHelper.CrossProduct<XmlElement>(wsdlBindingAlternatives, wsdlPortAlternatives, new YieldLimiter(this._importer._quotas.MaxYields, this._importer));
                }
                else
                {
            ...
            {
                return this._policyDictionary.ResolvePolicyReference(policyReference, contextPolicyAssertion);
            }

            class WsdlPolicyDictionary
            {
                readonly MetadataImporter _importer;
                readonly Dictionary<WsdlNS.ServiceDescription, Dictionary<string, XmlElement>> _embeddedPolicyDictionary = new Dictionary<WsdlNS.ServiceDescription, Dictionary<string, XmlElement>>();
                readonly Dictionary<string, XmlElement> _externalPolicyDictionary = new Dictionary<string, XmlElement>();
                readonly Dictionary<XmlElement, WsdlNS.ServiceDescription> _policySourceTable = new Dictionary<XmlElement, WsdlNS.ServiceDescription>();
                ...
                void AddEmbeddedPolicy(WsdlImporter importer, WsdlNS.ServiceDescription wsdl, XmlElement element)
                {
                    ...
                    {
                        string xPath = SoapInPolicyWorkaroundHelper.CreateXPathString(wsdl);
                        string warningMsg = SRP.Format(SRP.PolicyInWsdlMustHaveFragmentId, xPath);
                        WsdlImporter.SoapInPolicyWorkaroundHelper.LogImportWarning(importer._warnings, warningMsg, importer.Errors);
                        return;
                    }
                    ...
                    if (!_embeddedPolicyDictionary.TryGetValue(wsdl, out wsdlPolicyDictionary))
                    {
                        ...
                        this._embeddedPolicyDictionary.Add(wsdl, wsdlPolicyDictionary);
                    }
                ...
                void AddExternalPolicy(WsdlImporter importer, KeyValuePair<string, XmlElement> policyDocument)
                {
                    ...
                    {
                        string warningMsg = SRP.Format(SRP.UnrecognizedPolicyDocumentNamespace, policyDocument.Value.NamespaceURI);
                        WsdlImporter.SoapInPolicyWorkaroundHelper.LogImportWarning(importer._warnings, warningMsg, importer.Errors);
                        return;
                    }
                    ...
                    {
                        string warningMsg = SRP.Format(SRP.UnsupportedPolicyDocumentRoot, policyDocument.Value.Name);
                        WsdlImporter.SoapInPolicyWorkaroundHelper.LogImportWarning(importer._warnings, warningMsg, importer.Errors);
                        return;
                    }
                    ...
                    {
                        string warningMsg = SRP.Format(SRP.DuplicatePolicyDocumentSkipped, key);
                        WsdlImporter.SoapInPolicyWorkaroundHelper.LogImportWarning(importer._warnings, warningMsg, importer.Errors);
                        return;
                    }
                ...
                internal XmlElement ResolvePolicyReference(string policyReference, WsdlNS.ServiceDescription wsdlDocument)
                {
                    ...
                    Dictionary<string, XmlElement> wsdlPolicyDictionary;
                    if (!this._embeddedPolicyDictionary.TryGetValue(wsdlDocument, out wsdlPolicyDictionary))
                    {
                        return null;
                    }
                ...
                static readonly string s_wspPolicy = string.Format(CultureInfo.InvariantCulture, "{0}:{1}", MetadataStrings.WSPolicy.Prefix, MetadataStrings.WSPolicy.Elements.Policy);
                static readonly string s_xmlId = string.Format(CultureInfo.InvariantCulture, "{0}:{1}", MetadataStrings.Xml.Prefix, MetadataStrings.Xml.Attributes.Id);
                static readonly string s_wsuId = string.Format(CultureInfo.InvariantCulture, "{0}:{1}", MetadataStrings.Wsu.Prefix, MetadataStrings.Wsu.Attributes.Id);
